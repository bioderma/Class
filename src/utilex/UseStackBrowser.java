...
public class UseStackBrowser {
	...
	public static void goURL(String url) {
		//아래는 RunTime 클래스의 유용한 메서드중 OS 의 특정 exe를 동적으로 호출하여
		//실행하는 메서드를 사용합니다. 여기서 주어지는 파라미터가 윈도우에 잇고, 이름이
		//정확하다면 실행이 되어집니다.
		
		Runtime runtime = Runtime.getRuntime();
		
		try { runtime.exec("C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe "+ url);
		}catch (IOException e) {
			e.printStackTrace();
		}
		back.push(url);//back 안에 저장하기 (push)
