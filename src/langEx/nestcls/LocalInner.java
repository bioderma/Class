 ...
 * 상수 아니어도 지역 변수를 가질수 있게 되었습니다. 어려우면 상수만 가질수 있다라고 생각하세요.
 * 
...
public class LocalInner {
		...
		class InstanceInner{ //멤버 inner 클래스 정의
			int a = 100;
		}
		...
		void yourMethod(final int gg) {
			/*
			 * 로컬inner 클래스에서 접근 가능한 지역 변수는 반드시 final 이어야 합니다.
			 * 단 jdk 10x 버전 이후부터는 이 제한이 풀어졌습다.
			 * 
			 * */
	...
	public static void main(String[] args) {
			LocalInner outer = new LocalInner(); // 로컬이너 아우터 객체만들기 인스턴스
			
			
			//LocarInner 의 InnerInstance 타입의 instance 를 생성해보세요.
			LocalInner.InstanceInner in = outer.new InstanceInner();
			
			System.out.println(in.a);
			
			//지역Inner 객체를 수행하도록 만듭니다.
			outer.yourMethod(10);
	}
