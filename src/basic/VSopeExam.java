...
public class VSopeExam {
	...
	public static void main(String[] args) {
		...
		//초기화 되어지기 때문에 사용하는데 문제가 되질 않습니다.
		...
		//즉 static 이붙은 변수는 클래스 변수, none static 은 멤버필드(Instance 변수)라 합니다.
		//위 클래스 or 멤버필드는 순서대로 클래스가 메모리에 남아있는동안 살아있고,
		//멤버는 객체가 몸쓸객체로 변이되기 전까지는 살아있습니다.
		//지역(local) 변수는 메서드의 body 가 끝나면 삭제되고, 블락은 블락이 실행후
		//삭제 됩니다.
		int i; //main() 에서 처음 선언되었으므로 지역 변수입니다.
		i = 10;
		{
			int j = 2; //블락변수 선언
			int sum = i*j; //블락변수 선언..
		...
		/* System.out.println(i+"*" + j +"=" + sum); 블락변수 밖에서 선언되었기때문에 에러가뜸*/ 
